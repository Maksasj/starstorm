...
impl Weapon for OnyxBluster {
    fn shoot(&mut self, mut commands: &mut Commands, asset_server: &Res<SpriteSheet>, angle: f32, start_pos: Vec2, time: &Res<Time>) {
        ...
        if self.timer > self.speed {
            ...
            spawn_bullet(&mut commands, &handle, 6, start_pos, angle + 0.5235, Velocity::with(200.0, 0.0), self.damage);
            spawn_bullet(&mut commands, &handle, 6, start_pos, angle, Velocity::with(200.0, 0.0), self.damage);
            spawn_bullet(&mut commands, &handle, 6, start_pos, angle - 0.5235, Velocity::with(200.0, 0.0), self.damage);
